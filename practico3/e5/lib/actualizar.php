...
function actualizarMateria($id,$nombre_value,$profesor_value){
    ...
    if (($id != null) && (isset($_POST['nombre'])) && isset($_POST['profesor']) && isset($_POST['id_carrera'])) {
        try {
            $db = new PDO('mysql:host=localhost;dbname=universidad', 'root', 'root');
            $sentencia = $db->prepare("UPDATE materias 
                                        SET nombre=?,profesor=?,carrera=?
                                        WHERE id=?");

            $sentencia->execute(array($_POST['nombre'],$_POST['profesor'],isset($_POST['id_carrera']),$id));
